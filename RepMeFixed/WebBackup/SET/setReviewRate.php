...
$reviewRate = $_GET['review_rate'];  // Replace with the appropriate method to retrieve the review rate from your form
...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $stmt = $conn->prepare("UPDATE RepMe_Users SET `ReviewRate` = :reviewRate WHERE name = :name");
        $stmt->bindParam(':name', $searchName);
        $stmt->bindParam(':reviewRate', $reviewRate);
        $stmt->execute();

        echo "Review rate updated successfully.";
    } else {
